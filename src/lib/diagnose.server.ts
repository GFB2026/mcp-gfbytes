...
const RESOURCE_METADATA_RE =
  /resource_metadata\s*=\s*(?:"([^"]+)"|'([^']+)'|([^\s,;]+))/i;

const TIMEOUT_MS = 12_000;
const MAX_BODY = 65_536;
const MAX_HOPS = 5;
const UA = `mcp-oauth-connect/${DIAGNOSE_VERSION}`;
...
function initBody(protocolVersion: string): string {
  return JSON.stringify({
    ...
    params: {
      protocolVersion,
      capabilities: {},
      clientInfo: { name: "mcp-oauth-connect-diagnose", version: DIAGNOSE_VERSION },
      _meta: { "io.modelcontextprotocol/protocolVersion": protocolVersion },
    },
  });
}

const INIT_2025 = initBody("2025-03-26");
const INIT_2026 = initBody("2026-07-28");
...
function assertPublicHttps(raw: string): URL {
  ...
  if (parsed.protocol !== "https:") {
    throw new Error("URL must be https (Connectors UI will not complete OAuth over http)");
  }
...
function splitOriginMcp(base: string): { origin: string; mcpUrl: string } {
  const parsed = new URL(base.trim());
  const origin = originOf(parsed);
  const path = (parsed.pathname || "").replace(/\/$/, "");
  return { origin, mcpUrl: origin + (path || "/mcp") };
}

function prmPathUrl(origin: string, mcpUrl: string): string {
...
function resourceMetadataUrl(www: string): string | null {
...
function loadJson(body: Uint8Array): Record<string, unknown> {
  try {
    const data = JSON.parse(decodeBody(body) || "{}") as unknown;
...
function jsonrpcResult(body: Uint8Array): Record<string, unknown> | null {
...
function looksLikeMcpHello(result: Record<string, unknown> | null): boolean {
...
async function request(
  method: string,
  url: string,
  opts?: { data?: string; extra?: Record<string, string> },
): Promise<Probe> {
...
async function followSameHost(
  ...
  startUrl: string,
  opts?: { data?: string; extra?: Record<string, string> },
): Promise<{ probe: Probe; hops: RedirectHop[]; crossed: boolean }> {
  const hops: RedirectHop[] = [];
  let current = startUrl;
  for (let i = 0; i < MAX_HOPS; i++) {
    const probe = await request(method, current, opts);
    if (!(probe.status >= 300 && probe.status < 400 && probe.location)) {
      return { probe: { ...probe, url: current }, hops, crossed: false };
    }
    let next: string;
    try {
      next = new URL(probe.location, current).toString();
    } catch {
      return { probe, hops, crossed: false };
    }
    hops.push({ from: current, to: next, status: probe.status, method });
    if (hostsDiffer(current, next)) {
      return { probe, hops, crossed: true };
    }
    current = next;
  }
...
function prmCheck(
  ...
  return {
    ...
    authorization_servers: Array.isArray(parsed.authorization_servers)
      ? parsed.authorization_servers.filter((s): s is string => typeof s === "string")
      : null,
    hint: ok ? null : "Need HTTP 200 JSON at this well-known path.",
  };
}

function buildFindings(report: DiagnoseReport): Finding[] {
  ...
  if (reg && !reg.ok) {
    findings.push({
      ...
      body:
        `The URL in the Connectors dialog (${report.registered_url}) redirects to ${report.mcp_url}. ` +
        `Claude, Cursor, and Grok send initialize as POST and often do not replay it. Paste the final URL.`,
    });
  }
  ...
  if (prmMatch && !prmMatch.ok) {
    findings.push({
      severity: "fail",
      title: "Resource identifier does not match this URL",
      body:
        `Protected-resource metadata names ${prmMatch.resource || "(missing)"} while Connectors bound ` +
        `${report.mcp_url}. The authorization server then issues a token for a different resource.`,
    });
  }
  ...
  if (post && !post.ok) {
    findings.push({
      ...
      body:
        post.hint ||
        "Need POST HTTP 401 + WWW-Authenticate Bearer resource_metadata, or HTTP 200 JSON-RPC initialize.",
    });
  } else if (post?.mode === "anonymous_discovery_2026-07-28") {
    findings.push({
      severity: "warn",
  ...
  if (get && !get.ok) {
    findings.push({
      ...
      body:
        get.hint ||
        "GET should be 401 (challenge) or 405 (POST-only). HTTP 200 without auth is what curl celebrates and Connectors rejects.",
    });
  }
  ...
  } else if (report.as_url.includes("openid-configuration")) {
    findings.push({
      severity: "warn",
  ...
  if (prmAny && !prmAny.ok) {
    ...
    if (originPrm && !originPrm.ok && pathPrm?.ok) {
      findings.push({
        severity: "warn",
    ...
    if (pathPrm && !pathPrm.ok && originPrm?.ok) {
      findings.push({
        severity: "warn",
...
export async function diagnose(base: string): Promise<DiagnoseReport> {
  let parsed: URL;
  try {
    ...
    return {
      ...
      ok: false,
      warnings: [],
      findings: [{ severity: "fail", title: "Bad URL", body: msg }],
      hops: [],
      checks: {},
      error: msg,
    };
  ...
  const setCheck = (key: string, check: CheckResult, gates = true) => {
    report.checks[key] = check;
    if (gates && !check.ok) report.ok = false;
  };

  try {
    const [asProbe, oidcProbe, prmOriginProbe, prmPathProbe, getFollow, postFollow, post26Follow] =
      await Promise.all([
    ...
    if (asProbe) {
      ...
      setCheck("authorization_server_metadata", {
        ...
        registration_endpoint:
          typeof asJson.registration_endpoint === "string" ? asJson.registration_endpoint : null,
        hint: asOk
          ? null
          : "Need JSON with issuer, authorization_endpoint, token_endpoint, and registration_endpoint.",
      });
    } else {
      setCheck("authorization_server_metadata", {
        ok: false,
    ...
    const methods = Array.isArray(asJson.code_challenge_methods_supported)
      ? asJson.code_challenge_methods_supported.filter((m): m is string => typeof m === "string")
      : [];
    ...
    const prmOrigin = prmOriginProbe
      ? prmCheck(prmOriginUrl, prmOriginProbe.status, prmOriginProbe.body, prmOriginProbe.location)
      : { ok: false, url: prmOriginUrl, status: 0, hint: "Resource metadata request failed.", location: null };
    setCheck("protected_resource_metadata", prmOrigin, false);

    const prmPath = prmPathProbe
      ? prmCheck(prmPathInitial, prmPathProbe.status, prmPathProbe.body, prmPathProbe.location)
      : {
          ok: false,
          url: prmPathInitial,
          status: 0,
    ...
    setCheck("protected_resource_metadata_any", {
      ...
      hint: prmAny
        ? null
        : "Need JSON at /.well-known/oauth-protected-resource and/or the path-appended form.",
    });

    const hops = [...getFollow.hops, ...postFollow.hops, ...post26Follow.hops];
    report.hops = hops;
    ...
    setCheck("mcp_no_cross_host_redirect", {
      ...
      hint: crossed
        ? "MCP URL redirects to a different host, which drops Authorization. Register the final URL."
        : null,
    });

    const finalMcp =
      postFollow.hops.length > 0
        ? postFollow.hops[postFollow.hops.length - 1]!.to
        : getFollow.hops.length > 0
          ? getFollow.hops[getFollow.hops.length - 1]!.to
          : initialMcp;
    report.mcp_url = finalMcp;

    const registeredIsFinal =
      initialMcp.replace(/\/$/, "") === finalMcp.replace(/\/$/, "");
    const firstHop = postFollow.hops[0] || getFollow.hops[0];
    setCheck("registered_url_is_final", {
      ok: registeredIsFinal,
      status: firstHop?.status ?? null,
      registered: initialMcp,
      final: finalMcp,
      hops: hops.length,
      hint: registeredIsFinal
        ? null
        : `Connectors posted to ${initialMcp} and followed a same-host redirect to ${finalMcp}. Paste the final URL.`,
    });
    ...
    setCheck("unauthenticated_mcp_get", {
      ...
      hint: getOk
        ? null
        : "GET should be 401 (challenge) or 405 (POST-only). HTTP 200 without auth is the curl-pass / UI-fail pattern.",
    });

    const postProbe = postFollow.probe;
    const post26Probe = post26Follow.probe;
    const www =
      postProbe.headers["www-authenticate"] ||
      post26Probe.headers["www-authenticate"] ||
      getProbe.headers["www-authenticate"] ||
      "";
    let meta = resourceMetadataUrl(www);
    let challengeOk =
      postProbe.status === 401 && www.toLowerCase().includes("bearer") && Boolean(meta);
    ...
    const anonOk =
      (post26Probe.status === 200 || postProbe.status === 200) && looksLikeMcpHello(hello);

    let mode: "version_gate" | "anonymous_discovery_2026-07-28" | "oauth_challenge" | "miss";
    ...
    setCheck("unauthenticated_mcp_post", {
      ...
      server_name:
        hello &&
        typeof hello.serverInfo === "object" &&
        hello.serverInfo &&
        typeof (hello.serverInfo as { name?: unknown }).name === "string"
          ? (hello.serverInfo as { name: string }).name
          : null,
      protocolVersion: typeof hello?.protocolVersion === "string" ? hello.protocolVersion : null,
      hint:
        mode !== "miss"
          ? null
          : "Need POST HTTP 401 plus WWW-Authenticate: Bearer with an absolute resource_metadata URL, or HTTP 200 JSON-RPC initialize.",
    });
    ...
    setCheck("resource_metadata_absolute", {
      ...
      hint: absOk
        ? null
        : "resource_metadata must be an absolute HTTPS URL on the 401 WWW-Authenticate header, or JSON at origin /.well-known/oauth-protected-resource.",
    });

    const prmResource =
      (typeof prmOrigin.resource === "string" && prmOrigin.resource) ||
      (typeof prmPath.resource === "string" && prmPath.resource) ||
      null;
    let matchOk = true;
    ...
    if (prmResource) {
      try {
        ...
        if (!matchOk) {
          matchHint = `Resource metadata names ${prmResource} but the MCP host is ${mcpHost}. Connectors bind the resource identifier to the URL you paste.`;
        }
      } catch {
        matchOk = false;
